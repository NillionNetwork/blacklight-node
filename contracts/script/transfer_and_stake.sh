...
# Transfer and Stake Script
# ==========================
# Transfers TEST tokens from deployer, stakes them for an operator, and transfers ETH for gas fees.
# Note: The deployer must already have sufficient TEST tokens in their wallet.
#
# Usage: ./transfer_and_stake.sh <operator_address> <token_amount> <eth_amount>
#
...
if [ $# -lt 3 ]; then
    ...
    echo "  token_amount      - Amount of TEST tokens to transfer and stake (in wei)"
    ...
    echo ""
    echo "Note: Deployer must already have sufficient TEST tokens in their wallet."
    echo ""
    echo "Examples:"
...
if [ $# -eq 4 ]; then
    ...
    ENV_FILE="script/transfer_and_stake.env"
    OPERATOR_ADDRESS="$1"
    TOKEN_AMOUNT="$2"
    ETH_AMOUNT="$3"
else
...
echo -e "${GREEN}=== Transfer and Stake Script ===${NC}"
...
echo ""
echo -e "${YELLOW}Note:${NC} This script will transfer tokens from the deployer's existing balance."
echo -e "${YELLOW}      Ensure the deployer has sufficient TEST tokens before proceeding.${NC}"
echo ""
...
forge script script/TransferAndStake.s.sol:TransferAndStake \
    --rpc-url "$RPC_URL" \
    --broadcast \
    -vvv
