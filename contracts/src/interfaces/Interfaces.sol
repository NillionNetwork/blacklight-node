...
interface ICommitteeSelector {
    function selectCommittee(bytes32 workloadKey, uint8 round, uint32 committeeSize)
        external
        view
...
interface ISlashingPolicy {
    ...
    function onRoundFinalized(bytes32 workloadKey, uint8 round, Outcome outcome, address[] calldata committeeMembers)
        external;
}
...
interface IRewardPolicy {
    function onWorkloadValidated(bytes32 workloadKey, uint8 round, address[] calldata committeeMembers) external;
}
