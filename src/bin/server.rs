...
use nilav::{
    choose_k, load_config_from_path, AssignmentMsg, Builder, BuilderMeasurement, Config, Htx,
    NilCcMeasurement, NilCcOperator, WorkloadId,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "[sim] config: validators_per_slot={}, approve_threshold={}",
        config.election.validators_per_slot, config.election.approve_threshold
    );
    ...
    tokio::spawn(async move {
        ...
        let mut slot: u64 = 0;
        let mut current_workload: u64 = 1000;
        loop {
            ticker.tick().await;
            slot += 1;
            let prev = current_workload;
            current_workload += 1;
            ...
            let htx = Htx {
                ...
                nil_cc_operator: NilCcOperator {
                    id: 4,
                    name: "CC Cloud".to_string(),
                },
                builder: Builder {
                    id: 94323,
                    name: "0xlala".to_string(),
                },
                nil_cc_measurement: NilCcMeasurement {
                    url: "https://nilcc.nillion.com/...".to_string(),
                    version: "v1.3.0".to_string(),
                    v_cpus: 2,
                    gpus: 1,
                },
                builder_measurement: BuilderMeasurement {
                    url: "https://github.com/0xlala/...".to_string(),
                },
            };
            ...
            let assigned = choose_k(
                &nodes,
                nodes.len().min(config_clone.election.validators_per_slot),
            );
            println!(
