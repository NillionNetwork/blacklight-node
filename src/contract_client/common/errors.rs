...
//!    - Currently supports: `StakingOperatorsErrors`, we need to add more for the custom contracts such as NilUVRouterErrors
...
//!         → "NilUV: HTX already exists" (human-readable!)
//! ```
//!
//! ## Example
//!
...
//! You now see:
//! ```text
//! error: NilUV: HTX already exists
//! ```
//!
//! ## Main Entry Points
//!
...
// Note: NilUVRouter currently uses require() with string messages, not custom errors.
// When custom errors are added to the contract, they will be automatically available
// via NilUVRouter::NilUVRouterErrors and should be added here.
...
/// println!("Error: {}", decoded); // "NilUV: HTX already exists"
/// ```
...
fn try_extract_from_string(error_str: &str) -> Option<DecodedRevert> {
    ...
    // Some RPC providers return: `execution reverted: NilUV: HTX already exists`
...
mod tests {
    ...
    fn test_decode_error_string() {
        // "NilUV: unknown HTX" encoded as Error(string)
        // Selector: 08c379a0
        // Offset:   0000...0020 (32 bytes)
        // Length:   0000...0012 (18 bytes = "NilUV: unknown HTX".len())
