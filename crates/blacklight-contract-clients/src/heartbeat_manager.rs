...
impl<P: Provider + Clone> HeartbeatManagerClient<P> {
    ...
    pub fn new(provider: P, config: super::ContractConfig, tx_lock: Arc<Mutex<()>>) -> Self {
        ...
        Self {
            ...
            contract,
            submitter,
            block_lookback: DEFAULT_LOOKBACK_BLOCKS,
        }
    }

    /// Set the number of blocks looked back by default.
    pub fn with_block_lookback(mut self, blocks: u64) -> Self {
        self.block_lookback = blocks;
        self
    }
    ...
    pub async fn get_htx_submitted_events(&self) -> Result<Vec<HeartbeatEnqueuedEvent>> {
        self.get_htx_submitted_events_with_lookback(self.block_lookback)
            .await
    }
    ...
    pub async fn get_htx_assigned_events(&self) -> Result<Vec<RoundStartedEvent>> {
        self.get_htx_assigned_events_with_lookback(self.block_lookback)
            .await
    }
    ...
    pub async fn get_htx_responded_events(&self) -> Result<Vec<OperatorVotedEvent>> {
        self.get_htx_responded_events_with_lookback(self.block_lookback)
            .await
    }
