...
// ============================================================================
// All Known Contract Errors (deduplicated)
// ============================================================================
//
...
pub use AllErrors::AllErrorsErrors;
    ...
    F: FnOnce(&Bytes) -> Option<DecodedRevert>,
{
    ...
    // Step 3: Try all known contract errors
    if let Ok(err) = AllErrorsErrors::abi_decode(data) {
        return DecodedRevert::CustomError(format!("{err:?}"));
    }

    // Step 4: Unknown error - return raw bytes so user can debug
    ...
    F: Fn(&Bytes) -> Option<DecodedRevert>,
{
    ...
    // Some RPC providers return: `execution reverted: contract: request already exists`
    if error_str.contains("execution reverted")
        && let Some(idx) = error_str.find("execution reverted:")
    {
    ...
    fn test_decode_error_string() {
        ...
        match decoded {
            DecodedRevert::ErrorString(msg) => {
                assert_eq!(msg, "blacklight: unknown HTX");
            }
