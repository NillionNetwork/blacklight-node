...
pub fn display_wallet_status(
    ...
    let status_message = match status {
        WalletStatus::Created => {
            format!(
                "❗ Please fund this address with at least {min_eth_formatted} ETH and stake NIL tokens to continue ❗"
            )
        }
    ...
    table.add_row(Row::new(vec![
        TableCell::builder(status_message)
            .col_span(2)
            .alignment(CellAlignment::Center)
            .build(),
    ]));
...
mod tests {
    ...
    fn parse_wallet(private_key: &str) -> Result<PrivateKeySigner> {
    ...
    fn test_load_wallet() {
        ...
        let wallet = parse_wallet(private_key).unwrap();
